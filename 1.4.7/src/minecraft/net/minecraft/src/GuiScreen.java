...
public class GuiScreen extends Gui
{
    ...
    private int nbtInt(NBTTagCompound tag, String key, int def){
    	return tag.hasKey(key)?tag.getInteger(key):def;
    }
    
    private boolean nbtBool(NBTTagCompound tag, String key, boolean def){
    	return tag.hasKey(key)?tag.getBoolean(key):def;
    }
    
    protected void loadSprint(){
    	java.io.File file = new java.io.File(mc.mcDataDir,"sprint.nbt");
    	if (!file.exists())return;
    	try{
    		NBTTagCompound tag = CompressedStreamTools.readCompressed(new java.io.FileInputStream(file)).getCompoundTag("Data");
    		if (tag==null)return;
    ...
    protected void saveSprint(){
    	...
    	try{
    		CompressedStreamTools.writeCompressed(fintag,new java.io.FileOutputStream(new java.io.File(mc.mcDataDir,"sprint.nbt")));
    	}catch(Exception e){
        ...
        if (par3 == 0)
        {
            for (int var4 = 0; var4 < this.controlList.size(); ++var4)
            {
                ...
                if (var5.mousePressed(this.mc, par1, par2))
                {
                    ...
                    // CHANGE
                    if (this instanceof GuiControls&&var5.id==199)mc.displayGuiScreen(new GuiSprint(this));
    ...
    public void drawDefaultBackground()
    {
        ...
        if ((width!=prevWidth||height!=prevHeight)&&this instanceof GuiControls){
        	...
        	if (!fromBs){
				...
				if (!isForge)controlList.add(new GuiButton(199,width/2-100,height/6+146+var4,"Better Sprinting")); // 168-22
				else controlList.add(new GuiButton(199,width/2-155,newY,150,20,"Better Sprinting"));
        	}
			//controlList.add(new GuiSmallButton(199, (width/2-155)+var3%2*160, this.height/6+24*(var3>>1), 70, 20, "Sprint menu"));
