...
public class EntityPlayerSP extends EntityPlayer
{
    ...
    public void onLivingUpdate()
    {
        ...
        else
        {
            ...
            if (((GuiScreen.allowDoubleTap && !isSprinting())||!GuiScreen.allowDoubleTap) && onGround && var4 && !isUsingItem() && !isPotionActive(Potion.blindness)){	
            	setSprinting(state);
            }
            GuiScreen.held=state;
            
            if (GuiScreen.allowDoubleTap && !GuiScreen.held && GuiScreen.stoptime==0 && onGround && !var3 && movementInput.moveForward >= var2 && !isSprinting() && var4 && !isUsingItem() && !isPotionActive(Potion.blindness)){
            ...
            if (GuiScreen.flyingBoost>0){
	            if (state&&capabilities.isFlying){
	            	capabilities.setFlySpeed(0.05F*(1+GuiScreen.flyingBoost));
					if (capabilities.isFlying){
						if (movementInput.sneak){
							motionY-=0.15D*GuiScreen.flyingBoost;
						}
            ...
            if (GuiScreen.keyBindSprintMenu.pressed)mc.displayGuiScreen(new GuiSprint(null));
            ...
            {
                if ((GuiScreen.canRunInAllDirs(mc)&&GuiScreen.allowAllDirs)==false||(movementInput.moveForward==0F&&movementInput.moveStrafe==0F))setSprinting(false); // CHANGED LINE
            }
