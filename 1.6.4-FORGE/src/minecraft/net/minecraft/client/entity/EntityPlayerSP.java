...
{
	public boolean BetterSprintingClassCheck; // CHANGED LINE
    ...
    public void onLivingUpdate()
    {
        ...
        else
        {
            ...
            if (((GuiSprint.allowDoubleTap && !isSprinting())||!GuiSprint.allowDoubleTap) && onGround && flag2 && !isUsingItem() && !isPotionActive(Potion.blindness)){	
            	setSprinting(state);
            }
            GuiScreen.held=state;
            
            if (GuiSprint.allowDoubleTap && !GuiScreen.held && GuiScreen.stoptime==0 && onGround && !flag1 && movementInput.moveForward >= f && !isSprinting() && flag2 && !isUsingItem() && !isPotionActive(Potion.blindness)){
            ...
            if (GuiSprint.keyBindSprintMenu.pressed)mc.displayGuiScreen(new GuiSprint(null));
