...
public final class TransformerEntityPlayerSP implements IClassTransformer{
	private static final String[] NAMES_ONLIVINGUPDATE = new String[]{ "m", "onLivingUpdate" };
	...
	private void transformOnLivingUpdate(MethodNode method){
		...
		String[] mtdIsCurrentViewEntity = new String[]{ "isCurrentViewEntity", "A" };
		...
		for(int index = insertionPoint; index < instrcount; index++){
			...
			if (instruction.getOpcode() == INVOKEVIRTUAL){
				...
				if (ArrayUtils.contains(mtdIsCurrentViewEntity, methodInsn.name) && methodInsn.desc.equals("()Z")){
					skipPoint = index - 8;
					break;
				}
			}
	...
	private void transformOnLivingUpdateEnd(MethodNode method){
		...
		String[] mtdSendPlayerAbilities = new String[]{ "sendPlayerAbilities", "t" };
