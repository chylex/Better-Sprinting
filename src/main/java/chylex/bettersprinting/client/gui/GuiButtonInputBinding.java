...
public final class GuiButtonInputBinding extends GuiButtonCustomInput<GuiButtonInputBinding>{
	...
	public void updateKeyBindingText(){
		...
		if (isSelected){
			setMessage(TextFormatting.WHITE + "> " + TextFormatting.YELLOW + binding.func_197978_k() + TextFormatting.WHITE + " <");
		}
		else if (hasConflict){
			setMessage((hasOnlyModifierConflict ? TextFormatting.GOLD : TextFormatting.RED) + binding.func_197978_k());
		}
		else{
			setMessage(binding.func_197978_k());
		}
	}
}
