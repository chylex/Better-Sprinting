...
final class PlayerLogicHandler{
	...
	private boolean wasSneaking; // not present in 1.7.10, but backported in case it fixes any bug
	...
	public void updateLiving(){
		...
		if (ClientModManager.isModDisabled()){
			if (player.onGround && !wasSneaking && !wasMovingForward && movementInput.moveForward >= 0.8F && !player.isSprinting() && enoughHunger && !isSprintBlocked){
				if (player.sprintToggleTimer <= 0 && !ClientModManager.keyBindSprintHold.getIsKeyPressed()){
			...
			if (!player.isSprinting() && movementInput.moveForward >= 0.8F && enoughHunger && !isSprintBlocked && ClientModManager.keyBindSprintHold.getIsKeyPressed()){
				player.setSprinting(true);
			}
		}
		else{
		...
		if (player.capabilities.isFlying){
			double speed = ClientModManager.canBoostFlying() ? 3D * player.capabilities.getFlySpeed() : 0.15D;
